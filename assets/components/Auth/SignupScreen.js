...
//       if (route.params.experiencesJson) setSkillsData(JSON.parse(route.params.experiencesJson));
...
//   const goToSkills = () => {
//     // Pass current state so it can be returned safely without loss
...
//     // Fixed: Standardize dynamic empty strings for clean edit evaluation instead of breaking hashes with placeholder strings
...
//       const expJson = route.params?.experiencesJson || (skillsData ? JSON.stringify(skillsData) : "[]");
//       formData.append('experiencesJson', expJson);

//       // Extract dynamic Primary Category tracking identification securely out of parsed array layout structures
//       let extractedCatId = "1";
//       try {
//         const parsedExps = JSON.parse(expJson);
//         if (parsedExps && parsedExps.length > 0 && parsedExps[0].CategoryId) {
//           extractedCatId = parsedExps[0].CategoryId.toString();
//         } else if (route.params?.initialData?.categoryId) {
//           extractedCatId = route.params.initialData.categoryId.toString();
//         }
//       } catch (e) {
//         console.log("Error parsing dynamic category context fallback parameters:", e);
//       }
//       formData.append('CategoryId', extractedCatId);
//     }

//     // 3. Handle Binary Target Image Attachment Pipelines
...
//         });
//       } else {
//         NotificationHelper.showError(result.message || "Something went wrong during validation routing mapping.");
//       }
//     } catch (error) {
//       console.error("Auth Action Error:", error);
//       NotificationHelper.showError("Cannot reach backend context servers.");
//     } finally {
//       setIsLoading(false);
//     }
//   };
...
//         scrollEnabled={true} // Fixed: Allow dynamic scaling and user scrolling space on Step 1 layouts
//       >
//         {!route.params?.isEdit && (
//           <>
...
//       // Extract dynamic Primary Category ID safely out of the array layout structures
//       let extractedCatId = "1"; 
//       if (skillsData.length > 0 && skillsData[0].CategoryId) {
//         extractedCatId = skillsData[0].CategoryId.toString();
//       } else if (route.params?.initialData?.categoryId) {
//         extractedCatId = route.params.initialData.categoryId.toString();
//       }
//       formData.append('CategoryId', extractedCatId);

//       // FIX: Map individual experience fields into standard form data objects for model binding mapping
//       skillsData.forEach((exp, index) => {
//         formData.append(`Experiences[${index}].WorkAt`, exp.WorkAt);
//         formData.append(`Experiences[${index}].ExpDetail`, exp.ExpDetail);
//         formData.append(`Experiences[${index}].Duration`, exp.Duration);
//         formData.append(`Experiences[${index}].CategoryId`, exp.CategoryId.toString());
//         formData.append(`Experiences[${index}].SkillsId`, exp.SkillsId.toString());
//       });
//     }
...
const handleSignup = async () => {
  ...
  if (role === 'Worker') {
    ...
    // FIXED: Convert skillsData back to a pure string matching the expected parameter name on the C# controller
