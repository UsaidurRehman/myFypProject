...
const SignupScreen = ({ navigation, route }) => {
  ...
  useEffect(() => {
    ...
    if (route.params?.skillsCompleted) {
      ...
      if (route.params.cnic) setCnic(route.params.cnic);
      if (route.params.salary) setSalary(route.params.salary);
      ...
      if (route.params.confirmPassword) setConfirmPassword(route.params.confirmPassword);
      if (route.params.selectedImage) setSelectedImage(route.params.selectedImage);
  ...
  const goToSkills = () => {
    // Pass current state so it can be returned without loss
    navigation.navigate('AddSkills', {
      ...route.params, // IMPORTANT: Preserve initialData, isEdit, etc.
      name, age, phone, cnic, salary, email, address, password, confirmPassword,
      role, step, selectedImage, gender, bio,
      existingExperiences: skillsData // Pass existing skills to pre-populate
    });
  };
  ...
  const handleSignup = async () => {
    ...
    if (isEdit) {
      ...
      if (!initialId) {
        console.error("Edit mode error: No initial ID found in params", route.params);
    ...
    formData.append('Password', password || "********"); // Use placeholder if empty in edit
    ...
    if (role === 'Worker') {
      ...
      formData.append('CategoryId', route.params?.categoryId || route.params?.initialData?.categoryId || "1");
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView
        ...
        scrollEnabled={!(role === 'Worker' && step === 1)}
      >
        {!route.params?.isEdit && (
          <>
            ...
            <View style={styles.buttonRow}>
              ...
              <TouchableOpacity style={[styles.actionButton, styles.blueBtn]} onPress={() => role === 'Client' ? handleSignup() : setStep(2)}>
                {isLoading ? (
                  ...
                  <Text style={styles.buttonText}>{role === 'Client' ? 'Signup' : 'Next'}</Text>
                )}
              </TouchableOpacity>
            </View>
          </>
